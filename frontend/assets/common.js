...
function rpUpdateAuthNav() {
  ...
  if (user && rpGetToken()) {
    ...
    name.style.cssText = 'color:var(--text-dim);font-family:var(--font-mono);font-size:12px;';
    name.textContent = user.full_name || user.email;

    const logoutBtn = document.createElement('button');
    logoutBtn.className = 'auth-nav-btn';
    logoutBtn.style.cssText = 'background:none;border:1px solid var(--border-strong);color:var(--text-dim);border-radius:var(--radius-sm);padding:7px 14px;font-size:12px;cursor:pointer;font-family:var(--font-display);font-weight:500;transition:all 0.15s;';
